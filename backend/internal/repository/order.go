...
func (r *Order) GetByUser(ctx context.Context, userId int, limit int, offset int) ([]model.Order, error) {
	rows, err := r.db.Pool.Query(
		ctx,
		"SELECT order_id, user_id, product_id, quantity, created_at FROM orders WHERE user_id = $1 ORDER BY created_at DESC LIMIT $2 OFFSET $3;",
		userId, limit, offset,
	)
	if err != nil {
		return nil, err
...
// Get orders by product for a user with pagination
func (r *Order) GetByProduct(ctx context.Context, userId int, productId int, limit int, offset int) ([]model.Order, error) {
	rows, err := r.db.Pool.Query(
		ctx,
		"SELECT order_id, user_id, product_id, quantity, created_at FROM orders WHERE user_id = $1 AND product_id = $2 ORDER BY created_at DESC LIMIT $3 OFFSET $4;",
		userId, productId, limit, offset,
	)
	if err != nil {
		return nil, err
	}
